...
bool MapOverlayUiRenderer::EnsureResources(ID2D1RenderTarget* renderTarget, IDWriteFactory* writeFactory)
{
    ...
    if (!CreateTextFormat(writeFactory, L"Segoe UI", DWRITE_FONT_WEIGHT_SEMI_BOLD, 15.0f, &m_titleFormat))
        return false;
    if (!CreateTextFormat(writeFactory, L"Segoe UI", DWRITE_FONT_WEIGHT_SEMI_BOLD, 42.0f, &m_largeFormat))
        return false;
    m_largeFormat->SetTextAlignment(DWRITE_TEXT_ALIGNMENT_CENTER);
    m_largeFormat->SetParagraphAlignment(DWRITE_PARAGRAPH_ALIGNMENT_CENTER);
