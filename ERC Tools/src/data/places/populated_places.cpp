...
namespace
{
    ...
    bool TryParseCompactPlace(const json& value, PopulatedPlace& place)
    {
        if (!value.is_array() || value.size() < 4)
            return false;

        if (value[0].is_string())
            place.name = Trim(Utf8ToWide(value[0].get<std::string>()));
        if (value[1].is_string())
            place.country = Trim(Utf8ToWide(value[1].get<std::string>()));
        if (!TryJsonDouble(value[2], place.latitude) ||
            !TryJsonDouble(value[3], place.longitude))
        {
            return false;
        }
        if (value.size() > 4)
            TryJsonDouble(value[4], place.population);

        return !place.name.empty() &&
            place.latitude >= -90.0 && place.latitude <= 90.0 &&
            place.longitude >= -180.0 && place.longitude <= 180.0;
    }

    std::wstring PopulatedPlaceMergeKey(const PopulatedPlace& place)
    {
        const long long lat = std::llround(place.latitude * 1000.0);
        const long long lon = std::llround(place.longitude * 1000.0);
        return ToLower(Trim(place.name)) + L"|" +
            std::to_wstring(lat) + L"|" + std::to_wstring(lon);
    }
}
...
std::vector<PopulatedPlace> ParsePopulatedPlacesGeoJson(const std::string& body, std::wstring* errorOut)
{
    ...
    try {
        ...
        if (root.is_object() && root.contains("places") && root["places"].is_array()) {
            const json& compactPlaces = root["places"];
            out.reserve(compactPlaces.size());
            for (const auto& value : compactPlaces) {
                PopulatedPlace place;
                if (TryParseCompactPlace(value, place))
                    out.push_back(std::move(place));
            }
            if (out.empty() && errorOut)
                *errorOut = L"Populated places cache contains no usable places.";
            return out;
        }
...
std::vector<PopulatedPlace> ParseSupplementalPopulatedPlacesJson(
    ...
    std::wstring* errorOut)
{
    ...
    try {
        ...
        if (!root.is_array()) {
            if (errorOut)
                *errorOut = L"Supplemental populated places data is not a JSON array.";
            return out;
        }
        ...
        for (const auto& item : root) {
            ...
            PopulatedPlace place;
            place.name = PickPlaceString(item, { "name" });
            place.country = PickPlaceString(item, { "country" });

            auto latIt = item.find("lat");
            auto lonIt = item.find("lng");
            if (lonIt == item.end())
                lonIt = item.find("lon");
            if (place.name.empty() ||
                latIt == item.end() || lonIt == item.end() ||
                !TryJsonDouble(*latIt, place.latitude) ||
                !TryJsonDouble(*lonIt, place.longitude) ||
                place.latitude < -90.0 || place.latitude > 90.0 ||
                place.longitude < -180.0 || place.longitude > 180.0)
            {
                continue;
            ...
            auto populationIt = item.find("pop");
            if (populationIt == item.end())
                populationIt = item.find("population");
            if (populationIt != item.end())
                TryJsonDouble(*populationIt, place.population);

            out.push_back(std::move(place));
        }

        if (out.empty() && errorOut)
            *errorOut = L"Supplemental populated places data contains no usable places.";
    }
    catch (const std::exception& e) {
        if (errorOut)
            *errorOut = L"Supplemental populated places parse failed: " + Utf8ToWide(e.what());
        out.clear();
    }
    return out;
}

void MergePopulatedPlaces(
    std::vector<PopulatedPlace>& places,
    std::vector<PopulatedPlace> supplementalPlaces)
{
    std::unordered_set<std::wstring> keys;
    keys.reserve(places.size() + supplementalPlaces.size());
    for (const PopulatedPlace& place : places)
        keys.insert(PopulatedPlaceMergeKey(place));

    places.reserve(places.size() + supplementalPlaces.size());
    for (PopulatedPlace& place : supplementalPlaces) {
        if (keys.insert(PopulatedPlaceMergeKey(place)).second)
            places.push_back(std::move(place));
    }
}

std::string SerializePopulatedPlacesCache(
    const std::vector<PopulatedPlace>& places,
    std::wstring* errorOut)
{
    try {
        json root;
        root["format"] = "erc-populated-places-v3";
        root["sources"] = {
            "Natural Earth ne_10m_populated_places_simple",
            "GeoNames cities500 via lmfmaier/cities-json"
        };
        root["places"] = json::array();
        for (const PopulatedPlace& place : places) {
            root["places"].push_back({
                WideToUtf8(place.name),
                WideToUtf8(place.country),
                place.latitude,
                place.longitude,
                place.population
            });
        ...
        if (errorOut)
            *errorOut = L"Could not create populated places cache: " + Utf8ToWide(e.what());
        return {};
    }
}
...
{
    if (places.size() < 100000)
        return false;

    const size_t knownPopulationCount = static_cast<size_t>(std::count_if(
        places.begin(),
        places.end(),
        [](const PopulatedPlace& place) {
            return std::isfinite(place.population) && place.population >= 1.0;
        }));
    return knownPopulationCount >= 100000;
}
    ...
    double* distanceMilesOut)
{
    ...
    for (const PopulatedPlace& place : places) {
        if (!std::isfinite(place.population) ||
            place.population + 0.0001 < MaxValue(1.0, minimumPopulation))
        {
            continue;
        }
