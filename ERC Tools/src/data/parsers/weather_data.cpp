...
static std::wstring WeatherSystemIdFromDetailPath(std::wstring detailPath)
{
    detailPath = Trim(detailPath);
    if (detailPath.empty())
        return L"";

    std::replace(detailPath.begin(), detailPath.end(), L'\\', L'/');
    const size_t suffix = detailPath.find_first_of(L"?#");
    if (suffix != std::wstring::npos)
        detailPath.resize(suffix);

    const size_t slash = detailPath.find_last_of(L'/');
    std::wstring fileName = slash == std::wstring::npos
        ? detailPath
        : detailPath.substr(slash + 1);
    fileName = ToLower(Trim(fileName));
    if (fileName.size() > 5 &&
        fileName.compare(fileName.size() - 5, 5, L".html") == 0)
    {
        fileName.resize(fileName.size() - 5);
    }

    return fileName.empty() ? L"" : L"tsr|" + fileName;
}
...
std::vector<WeatherSystemEvent> ParseWeatherSystemEvents(const std::string& body, std::wstring& statusTextOut)
{
    ...
    for (std::wsregex_iterator it(html.begin(), html.end(), rowRe), end; it != end; ++it) {
        ...
        // TSR republishes the active-storm table with a new page timestamp,
        // while the per-storm detail filename remains stable for that cyclone.
        system.id = WeatherSystemIdFromDetailPath(system.detailPath);
        if (system.id.empty())
            system.id = ToLower(system.name) + L"|" + ToLower(system.basin);
