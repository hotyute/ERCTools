...
The receiver recreates the former public Traffic England Alerts contract. **Unplanned Only** contains current, confirmed, non-completed `INCIDENT` and `CONGESTION` records with a general-public presentation, public road identity, and display coordinates. **All Events** additionally includes the other Traffic England categories and preserves the former public-page exception for unconfirmed roadworks. Raw NTIS records remain in the stored snapshot with explicit eligibility flags, but the internal public endpoint never returns them to the map, side panel, or notification checks.

Event Data is change-driven, so a quiet feed does not invalidate the consolidated lifecycle cache or empty the public result. The endpoint exposes `sourceQuiet` after 10 minutes without a publication for monitoring, while retaining current records until a lifecycle update, full-refresh reconciliation, structured validity end, or the upper end of an unplanned event's public expected-clear band removes them. A full refresh replaces records at or before its publication time while preserving concurrent updates newer than the refresh, matching the NTIS subscriber reference implementation. `NTIS_EVENT_SNAPSHOT_MAX_AGE_SECONDS` changes the 600-second quietness threshold.
...
Events whose public text does not contain a road number are tagged as source-unresolved and retained for diagnostics, but are not part of the Traffic England-compatible public result. The receiver also checks coordinates asynchronously against the National Highways Network Model Link layer and caches the result for seven days. This enrichment can refine ambiguous route names, including upgrading `A1` to `A1(M)` where the Network Model identifies the motorway section, but a failed auxiliary lookup no longer hides an otherwise valid public event. A genuine Network Model push publication invalidates the cached matches.
